...
import { Plus, Edit, Trash2, Search, Medal } from "lucide-react";
import { cocurricularSubjectsApi } from "@/lib/api/core";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { toast } from "sonner";
import type { CocurricularSubject } from "@/types";

export function CocurricularSubjects() {
  const queryClient = useQueryClient();
  const [showForm, setShowForm] = useState(false);
  const [editingSubject, setEditingSubject] =
    useState<CocurricularSubject | null>(null);
  ...
  const { data: subjects = [], isLoading } = useQuery({
    queryKey: ["cocurricularSubjects"],
    queryFn: cocurricularSubjectsApi.getAll,
  });

  // Mutations
  const createSubjectMutation = useMutation({
    mutationFn: cocurricularSubjectsApi.create,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cocurricularSubjects"] });
  ...
  const updateSubjectMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      data: Partial<CocurricularSubject>;
    }) => cocurricularSubjectsApi.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cocurricularSubjects"] });
  ...
  const deleteSubjectMutation = useMutation({
    mutationFn: cocurricularSubjectsApi.delete,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cocurricularSubjects"] });
  ...
  const handleEdit = (subject: CocurricularSubject) => {
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">
                {editingSubject ? "Edit Activity" : "Add Activity"}
              </h3>
              <button
                onClick={closeForm}
